...
/*          printf("Bale Weight is: %d", rx_frame.data.u8[i]);
          ...
          if( (rx_frame.MsgID & 0xFF) == 0x02 ) // Verify The Source Address
          {
            if( ((rx_frame.MsgID >> 16) & 0xFF) == 0xFF ) // Proprietary B
            {
              if( ((rx_frame.MsgID >> 8) & 0xFF) == 0x51 ) // iSOCAN -> Application msg
              {
                printf(" Response is: from 0x%08x, DLC %d\n",rx_frame.MsgID,  rx_frame.FIR.B.DLC);
